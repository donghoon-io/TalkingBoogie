...
class InnerRoutine1ViewCell: UICollectionViewCell {
    ...
    @IBOutlet weak var imageView: UIImageView! {
        didSet {
            ...
            let routineTapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(routineImageTapped(tapGestureRecognizer:)))
            imageView.isUserInteractionEnabled = true
            imageView.addGestureRecognizer(routineTapGestureRecognizer)
        }
    }
    
    @IBOutlet weak var imageLabel: UILabel!
    
    @objc func routineImageTapped(tapGestureRecognizer: UITapGestureRecognizer) {
